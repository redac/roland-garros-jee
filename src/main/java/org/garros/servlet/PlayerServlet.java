...
public class PlayerServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// D'abord on r�cup�re l'id du joueur dont on veut la fiche		
		int id = Integer.parseInt(req.getParameter("id"));
		
		// Puis on fait la requete pour r�cup�rer tout ce dont on a besoin
		Player player = new Player();
		player = (Player) playerService.getPlayerById(id).get(0);
		
		
		// Enfin on affiche le r�sultat en fonction du Player
